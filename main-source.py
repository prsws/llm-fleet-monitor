#!/usr/bin/env python3
# Historical Source — kept unused for reference only.
# The live implementation is llm-fleet-monitor.py; this file is a stale earlier copy.
"""
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    p.add_argument("--json", dest="as_json", action="store_true", help="Emit the JSON envelope instead of text")
    p.add_argument("--verbose", action="store_true", help="Show full Whisper/Piper details (do not collapse languages/voices)")
    ...
    try:
        envelope = run_probe(rows, timeout=float(args.timeout))
    except Exception as e:
